# Releasing the bot-manager template

This repository publishes the public `tinyhat/agent-templates/bot-manager`
payload. It versions independently from every other Tinyhat repository.
...
gh release create "$TAG" \
  --repo tinyloophub/tinyhat--agent-templates--bot-manager \
  --title "$TAG" \
  --latest \
  --verify-tag \
...
This repository does not publish an owned npm package. If that changes, publish
finals to the `latest` dist-tag, candidates to `next`, and do not use `lts`
until a support owner and backport policy exist.
